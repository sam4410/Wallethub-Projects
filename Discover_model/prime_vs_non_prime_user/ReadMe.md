...
* __src__: The directory that contains the application to run in the container. See the next session for details about each of the files.

* __Smanage_local_docker.sh__: This is the script that is used to create the docker on local environment. This script creates the docker and pushes the image to ECR Repo. The current image is pushed to repository [434418615032.dkr.ecr.us-east-2.amazonaws.com/approval-odds-allcards-tmp]
